...
param(
    ...
    [switch][alias('a')]$all,
    [switch][alias('h')]$help,
    [ValidateSet("text", "bar", "textbar", "bartext")][string]$cpustyle = "bartext",
    [ValidateSet("text", "bar", "textbar", "bartext")][string]$memorystyle = "bartext",
    [ValidateSet("text", "bar", "textbar", "bartext")][string]$diskstyle = "bartext",
    [ValidateSet("text", "bar", "textbar", "bartext")][string]$batterystyle = "bartext",
    ...
    if ($image) {
        ...
        if (-not $logo) {
            if ($os -Like "*Windows 11 *") {
                ...
                $logo = "deathkanji" #note: default ascci art
            } else {
                $logo = "Windows 7"
            }
        }
...
function info_title {
    return @{
        title   = ""
        content = "▬▬ι═══════ﺤ  ${e}[1;31m{0}${e}[0m @  ${e}[1;31m{1}${e}[0m -═══════ι▬▬" -f [System.Environment]::UserName,$env:COMPUTERNAME #note: change title color
    }
}
...
function info_dashes_h {
    return @{
        title   = ""
        content = "${e}[;31m ハードウェア情報${e}[0m ──────────────────────"
        #content = "${e}[${t};31m ハードウェア情報${e}[0m ───────────────"
        #content = "┌───────────────${e}[;31m Hardware Information ${e}[0m───────────────┐"
    }
}
...
function info_dashes_s {
    return @{
        title   = ""
        content = "${e}[;31m ソフトウェア情報${e}[0m ──────────────────────"
        #content = "${e}[${t};31m ソフトウェア情報${e}[0m ───────────────"
        #content = "┌────────────────${e}[;31m Software Information ──────────────┐"
    }
}

# ===== DASHES-L =====
function info_dashes_l {
    return @{
        title   = ""
        content = "${e}[;31m 位置情報${e}[0m ──────────────────────────────"
        #content = "${e}[${t};31m 位置情報${e}[0m ───────────────"
        #content = "┌───────────────${e}[;33m Location Information ${e}[0m───────────────┐"
    }
}
